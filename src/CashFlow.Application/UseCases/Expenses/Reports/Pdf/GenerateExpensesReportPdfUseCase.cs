...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        foreach (var expense in expenses) {
            ...
            if (string.IsNullOrWhiteSpace(expense.Description) == false)
            {
                ...
                descriptionRow.Format.LeftIndent = 20;
    ...
    private Document CreateDocument(DateOnly mont)
    {
        var document = new Document();
        document.Info.Title = $"{ResourceReportsExpensesMessages.EXPENSES_FOR} {mont:Y}";
    ...
    private void CreateHeaderWithProfilePhotoAndName(Section page)
    {
        ...
        var pathName = Path.Combine(directoryName!, "Logo", "ProfilePhoto.png");

        row.Cells[0].AddImage(pathName);
        row.Cells[1].AddParagraph("Olá, Vanilson Sousa");
    ...
    private void CreateTotalSpentSection(Section page, DateOnly month, decimal totalExpenses)
    {
        ...
        var title = string.Format(ResourceReportsExpensesMessages.TOTAL_SPENT_IN, month.ToString("Y"));
    ...
    {
        cell.AddParagraph(ResourceReportsExpensesMessages.AMOUNT);
